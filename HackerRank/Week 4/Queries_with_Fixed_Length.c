 ...
 *
 */
int min_of_max_in_window(int* arr, int n, int d) {
    int* deque = (int*)malloc(n * sizeof(int));
    int front = 0, back = -1;
    int* max_in_window = (int*)malloc((n - d + 1) * sizeof(int));
    int idx = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        deque[++back] = i;
        if (i >= d - 1)
            max_in_window[idx++] = arr[deque[front]];
    }
    int result = INT_MAX;
    for (int i = 0; i < idx; i++) {
        if (max_in_window[i] < result)
            result = max_in_window[i];
    }
...
int* solve(int arr_count, int* arr, int queries_count, int* queries, int* result_count) {
    ...
    for (int i = 0; i < queries_count; i++) {
        results[i] = min_of_max_in_window(arr, arr_count, queries[i]);
    }
